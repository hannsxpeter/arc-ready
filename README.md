...
- **Tier 0 (orchestration).** Mode detection, intent capture, progress ledger, AGENTS.md emission, scope-fence enforcement.
- **Tier 1 (planning).** PRD, architecture, roadmap, stack decision. Each gates the next.
- **Tier 2 (building).** Repo scaffolding (production-grade structure, docs, CI/CD, quality tooling), application build (vertical slices, end-to-end-wired, no scaffolds, no placeholders).
...
- **Any AGENTS.md-aware harness** (Aider, Zed, Warp, Roo Code, Jules, Factory, Amp, Devin): the project's `AGENTS.md` (emitted by Tier 0 Step 0.6 if absent) names the artifact map; the harness consumes the artifacts directly.
...
**Building**: set up a repo, initialize a project, add documentation, set up CI, configure linting, add a README, set up GitHub Actions, make my repo professional, add contributing guidelines, set up release automation, dashboard, admin panel, internal tool, back office, control panel, analytics view, CRUD app.
...
| 0/2.1 | Cross-tool agent brief | `AGENTS.md` at project root |

## Status

arc-ready is at v0.1.0. The initial consolidation from aihxp/ready-suite is complete. Every named failure mode from the eleven source skills is preserved in the consolidated reference catalog under `references/<tier>/<skill>-antipatterns.md`.
