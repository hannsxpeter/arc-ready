...
This file defines the only artifact arc-ready produces: `.arc-ready/PROGRESS.md`. The ledger is the source of truth for "where in the arc this project sits," nothing more. The tiers own their own artifacts; PROGRESS.md is a denormalized read-cache for the auditor and the resume protocol.

Read this file at workflow Steps 1, 2, and 6.

## The single load-bearing principle

PROGRESS.md is **a view, not a database**. The source of truth is the union of `.{skill}-ready/` directories on disk. Every arc-ready turn begins by re-deriving state from disk; if PROGRESS.md disagrees with disk, **disk wins** and PROGRESS.md is corrected.

This discipline comes from Just / Make (`references/shared/RESEARCH-2026-04.md` Section 3.4): "the file system is the truth." A target is "done" if its output file exists and is non-empty. Without this discipline, arc-ready cannot defend against the phantom-resume class of failures cited in Anthropic claude-code issues #42338, #42260, #42309, NousResearch hermes-agent#17344, and paperclipai#635 (research Section 2.3).
...
### Skip declarations
<list of tiers the user explicitly opts out of, with reason>
- harden-ready: skip (one-day prototype; no real user surface)
- launch-ready: defer (internal-only tool; no public launch)

### Imports
<list of upstream artifacts the user already had before arc-ready started>
...
A markdown table, one row per tier. Every tier arc-ready knows about (the ten plus any future additions) appears as a row, even if the row's status is `pending` or `skipped`. **Silence is not a status.**
...
| `tier` | yes | string | Tier name (e.g., `prd-ready`). |
| `status` | yes | enum | One of `pending`, `in-flight`, `done`, `skipped`, `imported`, `failed`, `re-invoked`. Never blank. |
| `artifact_path` | yes | string | Full path of the tier's primary deliverable. Empty string allowed only when status is `pending` or `skipped`. |
| `invocation_ts` | conditional | ISO8601 | Timestamp the tier was invoked. Required for `in-flight`, `done`, `failed`, `re-invoked`. |
...
Each acceptance is dated, named, justified, and time-bounded. This is the harden-ready risk-register pattern; arc-ready inherits it for the launch-gate exception.
...
- **in-flight.** The tier has been invoked but has not yet returned a verified artifact. `invocation_ts` set; `verification_ts` not yet set.
- **done.** The tier completed and the artifact-exists check passed. Both `invocation_ts` and `verification_ts` set; `disk_state_hash` set.
...
- **imported.** An artifact at the expected path was already present before arc-ready ran (Step 0 detection) or the user explicitly imported it during the chain. `verification_ts` and `disk_state_hash` set; `invocation_ts` blank or set to the import action's timestamp; `notes` contains the source path or "pre-existing."
- **failed.** The tier was invoked, returned, but the artifact-exists check did not pass (file missing, file empty, file is the unmodified template). `invocation_ts` set; `verification_ts` blank; `notes` contains the failure mode and retry budget. The next arc-ready turn re-invokes the tier unless the budget is exhausted, in which case the user is asked to intervene.
- **re-invoked.** The user explicitly chose to re-run a previously-done tier because the input changed. The prior `done` row is preserved (with status moved to `re-invoked`) and a new row is added below it for the re-invocation. `notes` on the prior row contains the reason ("PRD changed; re-running architecture-ready").
...
3. If any `done` or `imported` row fails (a) or (b):
   ...
   - The next step is to re-invoke that tier, not to advance.
...
In Step 0, before any other action, arc-ready checks for pre-existing tier artifacts. The check is per-tier and uses the canonical artifact paths from `references/orchestration/sequencing-rules.md` per-tier upstream contract.

```
For each tier in the topological order:
  Check if the tier's primary artifact exists on disk.
  (For repo-ready, the check is multi-file: README.md at repo root AND
   any of .github/workflows/*.yml, .gitlab-ci.yml, .editorconfig,
   .repo-ready/SECURITY.md. The other nine tiers are single-file.)
  If yes:
    ...
    Ask the user (or note in PROGRESS.md if non-interactive):
      "I see .prd-ready/PRD.md already exists. Do you want to:
       ...
       (c) treat the existing file as out-of-scope
           and let arc-ready start fresh elsewhere"
  If no:
    Mark as `pending`.
```

The default on a fresh kickoff is to import existing artifacts (option a). The user can override per-tier. The Yeoman conflict-resolution semantic is the model (research Section 3.5): never silently overwrite existing files; always confirm.

### Canonical primary artifact paths per tier

| Tier | Canonical artifact (single-file unless noted) |
|---|---|
...
- **Any tier's deliverable copy-pasted.** PROGRESS.md points to artifact paths; it does not embed their contents.
...
- **Project-level decisions outside the arc.** "We decided to add a paid tier" belongs in roadmap-ready's `ROADMAP.md` or the project's own changelog, not in PROGRESS.md.

If a section in PROGRESS.md grows beyond schema-required content, scope leak is happening. Move the content to the correct tier's artifact and replace it with a path reference.
...
Arc completed on <ISO8601>. The project shipped with the following posture:

| Tier | Status | Artifact | Verified |
|---|---|---|---|
...
The arc is one-shot; phase work after this is not arc-ready's responsibility.
See references/shared/ORCHESTRATORS.md for the GSD integration pattern.
```

The audit-ledger view is durable. It survives arc-ready's removal from the harness. A new engineer joining the project six months later can read PROGRESS.md and reconstruct exactly which tiers were invoked, which were skipped, why, and where the artifacts live.

## Re-invocation discipline

When the user re-runs a previously-done tier because the input changed:
...
3. Mark every downstream tier's status as `re-invoked` if its upstream input changed in a way that affects it. (Example: re-running prd-ready likely affects architecture-ready, which affects roadmap-ready, etc. The arc-ready agent reasons about the dependency cascade and updates downstream rows accordingly.)
4. The user is informed of the cascade before arc-ready re-invokes downstream tiers; they may choose to manually edit downstream artifacts instead of full re-invocation.
...
PROGRESS.md belongs in the repo. Commit it. The next arc-ready session reads it from the working tree, not from a separate state directory. This makes the arc auditable in `git log` and reviewable in PRs.

The only exception: a pure prototype that will be thrown away within the day might run arc-ready without committing. In that case, PROGRESS.md is still authoritative for the session but is lost when the directory is deleted. This is the user's call; arc-ready does not enforce.

## Summary

The arc has one persistent artifact: `.arc-ready/PROGRESS.md`. The schema is small, the rules are mechanical, and the source of truth is always the file system. Every other rule about arc-ready collapses to: read disk, update PROGRESS.md, invoke the next tier, verify, repeat.
